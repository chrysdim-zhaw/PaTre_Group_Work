...
@nathani2022umwelt estimate that mobility behavior in Switzerland leads to at least 2.5 tonnes of CO₂e per capita, with over 1.5 tonnes resulting from domestic direct emissions—such as burning fuel in vehicle engines. As such, transport is the sector responsible for the largest share of GHG emissions. The United States presents a similar picture, where transport also ranks as the top emitting sector [@us2019us].

Without significant intervention, transport-related emissions are projected to increase by 16–50% by 2050. However, in scenarios aligned with the 1.5°C climate target, a 59% reduction in transport CO₂ emissions by 2050 (compared to 2020 levels) is required. More ambitious pathways, such as the “1.5°C Low Demand” scenario, envision reductions of up to 90% [@IPCC_2022_WGIII_Ch_10].
...
Moreover, @laube2011cross emphasized the importance of scale when analyzing movement trajectories. Their cross-scale perspective informed the treatment of domestic versus international travel in this study, particularly in identifying transitions between local commuting behavior and more dispersed travel patterns across national borders.
...
# Alle Transportmittel erfassen
all_labels <- unique(emissions_combined_filtered$activity_label)

# Transportmittel nach Dimis Gesamtdistanz sortieren
dimi_sorted <- emissions_combined_filtered %>%
  filter(person == "Dimi") %>%
  group_by(activity_label) %>%
  summarise(total_km = sum(total_distance_km, na.rm = TRUE), .groups = "drop") %>%
...
# Daten vorbereiten
plot_data <- emissions_combined_filtered %>%
  group_by(person, activity_label) %>%
  summarise(total_km = sum(total_distance_km, na.rm = TRUE), .groups = "drop") %>%
  mutate(activity_label = factor(activity_label, levels = dimi_sorted))

# Gesamtsummen berechnen
totals <- plot_data %>%
  group_by(person) %>%
  summarise(total = sum(total_km, na.rm = TRUE))

# Dynamischer Abstand für Text
max_total <- max(totals$total)
text_offset <- 0.05 * max_total

# Diagramm
ggplot(plot_data, aes(x = person, y = total_km, fill = activity_label)) +
  geom_bar(stat = "identity") +
  geom_text(
    data = totals,
    aes(x = person, y = total + text_offset, label = paste0(round(total), " km")),
    inherit.aes = FALSE,
    size = 5
  ) +
  labs(
...
<br><br>
Figure @fig-co2-km-bubble provides a comprehensive overview of both mobility behavior and resulting CO₂ emissions for Dimi and Ramon across various transport modes. The y-axis represents the total distance travelled (log scale), while the size of each bubble corresponds to the total CO₂ emissions (in kilograms) for that mode.
...
#| fig-cap: CO₂ emissions (bubble size) and distance travelled (log-scale) per transport mode for Dimi and Ramon
...
```{r}
# 1. Alle Transportmittel extrahieren
all_labels <- unique(emissions_combined_filtered$activity_label)

# 2. Sortierung nach Dimis totalen CO₂-Emissionen pro Transportmittel
dimi_sorted <- emissions_combined_filtered %>%
  filter(person == "Dimi") %>%
  group_by(activity_label) %>%
  summarise(total_co2 = sum(total_emission_kg, na.rm = TRUE), .groups = "drop") %>%
  right_join(tibble(activity_label = all_labels), by = "activity_label") %>%
  mutate(total_co2 = replace_na(total_co2, 0)) %>%
  arrange(total_co2) %>%
  pull(activity_label)

# 3. Plot-Daten aggregieren mit der gleichen Reihenfolge
plot_data <- emissions_combined_filtered %>%
  group_by(person, activity_label) %>%
  summarise(total_co2 = sum(total_emission_kg, na.rm = TRUE), .groups = "drop") %>%
  mutate(activity_label = factor(activity_label, levels = dimi_sorted))

# 4. Totale Emissionen pro Person (für Textanzeige)
totals <- plot_data %>%
  group_by(person) %>%
  summarise(total = sum(total_co2, na.rm = TRUE))

# 5. Dynamischer Abstand für Text
max_total <- max(totals$total)
text_offset <- 0.05 * max_total

# 6. Diagramm zeichnen
ggplot(plot_data, aes(x = person, y = total_co2, fill = activity_label)) +
  geom_bar(stat = "identity") +
  geom_text(
    data = totals,
    aes(x = person, y = total + text_offset, label = paste0(round(total), " kg CO₂")),
    inherit.aes = FALSE,
    size = 5
  ) +
  labs(
    title = "Total CO₂ Emissions by Person and Transport Mode",
    x = "Person",
    y = "CO₂ Emissions (kg)",
    fill = "Transport Mode"
  ) +
