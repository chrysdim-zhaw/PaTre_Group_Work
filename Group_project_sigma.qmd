...
ggplot(emissions_combined, aes(
  ...
  labs(
    title = "Gefahrene Kilometer nach Aktivität und Person",
    x = "Transportmittel",
    y = "Gefahrene Kilometer",
    size = "CO₂ Emissionen (kg)",
    color = "Person"
  ) +
  theme_minimal() +
  theme(
...
ggplot(emissions_combined, aes(
  ...
  geom_point(alpha = 0.8) +
  scale_size_continuous(range = c(4, 20)) +  # grössere Kreise
  labs(
    title = "Distance Traveled by Transport Type and Person",
    x = "Transport Mode",
    y = "Distance Traveled (km)",
    size = "CO₂ Emissions (kg)",
    color = "Person"
  ) +
  theme_minimal() +
  theme(
