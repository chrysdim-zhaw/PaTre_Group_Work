...
```{r}
pie_plot_dimi_days + pie_plot_ramon_days
```
...
co2_table %>%
  kable(
    caption = "CO₂-Faktoren nach Transportmittel",
    col.names = c("Transportmittel", "CO₂ (kg/km)"),
    digits = 3,
    align = "l"
  ) %>%
...
#co2_table %>%
 # gt() %>%
  #tab_header(title = md("**CO₂-Faktoren nach Transportmittel**")) %>%
#  cols_label(
#    activity_label = "Transportmittel",
#    co2_kg_per_km = "CO₂ (kg/km)"
#  ) %>%
...
# 📊 Plot 1: Emissionen
ggplot(emissions_padded, aes(x = activity_label, y = total_emission_kg, fill = person)) +
...
# 📊 Plot 2: Distanz
ggplot(emissions_padded, aes(x = activity_label, y = total_distance_km, fill = person)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), na.rm = TRUE) +
  scale_y_log10(
    limits = c(1, 10000),
...
final_plot <- ggplot(emissions_combined, aes(
  ...
  # Beschriftung mit besserer Rundung
  geom_text(
