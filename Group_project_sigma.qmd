...
1.	How do the movement patterns of the two students differ?
    ...
    b.	Which is the most frequently visited location? <span style="color:red;">or which are the most frequently visited locations?</span>
   ...
   <span style="color:red;">e. How does the movement patterns of the students differ compared to switzerland's average?</span>
    
## Data Collection
    
The primary data source for this study was Google Timeline tracking [@google2025timeline], which provided detailed movement patterns, including transportation modes, travel distances, and time spent at different locations. Mobility data was collected over the course of two months, ensuring that both students’ movement patterns were temporally aligned for accurate comparison. GHG emissions were quantified using SimaPro 10.1 [@simapro2025], drawing from the Ecoinvent v3.11 database [@ecoinvent2024], allowing for a precise assessment of the environmental impact associated with different transportation choices. This analysis incorporated contextual data on transport emission factors (e.g., CO₂ emissions per kilometer for cars, public transport, cycling, etc.) to improve accuracy. **Additional contextual information—such as university schedules and personal habits—was also considered to support interpretation of the mobility patterns. With all relevant data collected and processed, Google Timeline and SimaPro served as the foundation for the mobility and emissions analysis.**
...
Additional spatial analysis methods may include kernel density estimation (KDE) for hotspot detection and network analysis for transportation mode comparisons. <span style="color:red;">würde ich mal in klammern setzen</span> 
...
library(RColorBrewer) # For generating color palettes for plots and maps, especially categorical or sequential color scales.
...
```{r}
#| label: fig-map-dimi
#| fig-cap: "Visited locations of Dimi"
...
```{r}
#| label: fig-map-ramon
#| fig-cap: "Visited locations of Ramon"
#| 
leaflet_map_ramon
```
...
#| tbl-cap: CO₂ eq-factors per transportmode
